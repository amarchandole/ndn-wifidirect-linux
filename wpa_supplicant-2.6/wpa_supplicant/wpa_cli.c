...
static char *myp2p_disconnect[] = {"disconnect"};
static char *myp2p_find[] = {"p2p_find"};
static char *myp2p_peers[] = {"p2p_peers"};
static char *myp2p_connect[] = {"p2p_connect", "", "pbc", "auth", "freq=2412", "go_intent=7"};
//static char *dummy_args[] = {"p2p_find"};
...
static char* mac_to_ipv6(char *mac, char* ipv6) 
{
...
static void ndn_p2p(void)
{
	...
	printf("Trying p2p_find():\n");
	wpa_request(ctrl_conn, 1, myp2p_find);
	
	printf("Press ENTER to check your WiFi-Direct peers:\n");
	getchar();

	printf("Trying p2p_peers():\n");
	wpa_request(ctrl_conn, 1, myp2p_peers);
	...
	sleep(10); 	
	//printf("\n\n\nTry to ping this IPv6 address to check your connectivity: \n%s\n\n\n\n\n", ipv6);
}
...
{//printf("\n___________ %s _____________\n", __FUNCTION__);
	...
	if (interactive) {
		printf("Launching wpa_cli_interactive():\n");
		wpa_cli_interactive();
	} else {
		if (!global &&
		    wpa_cli_open_connection(ctrl_ifname, 0) < 0) {
			printf("Not interactive. Calling wpa_cli_open_connection() :\n");
		...
		if (action_file) {
			printf("Calling wpa_cli_action() :\n");
			wpa_cli_action(ctrl_conn);
		}
		else{
			printf("Calling wpa_request() :\n");
